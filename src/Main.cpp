  ...
  while (std::getline(file, line)) {
    ...
    if (headerBool == true) {
      if (line.find("element vertex") != std::string::npos) {
        ...
        std::istringstream vertexFind(line.substr(
      ...
      if (line.find("element face") != std::string::npos) {
        ...
        std::istringstream faceFind(line.substr(
            line.find(delimiter) + delimiter.length(), line.length()));
        faceFind >> faceCount;
  ...
  while (std::getline(file, line)) {
    ...
    if (line[0] == 'L') {
      ...
      if (obj < 0 || fFace < 0 || lFace < 0) {
        std::cerr << "Material file's object and/or faces should be positive "
                     "integers.\n";
        exit(-1);
      }
      if (d1 < 0 || d1 > 1 || d2 < 0 || d2 > 1 || d3 < 0 || d3 > 1 || k < 0 ||
          k > 1 || a < 0 || o < 0 || o > 1) {
        std::cerr << "Material properties must be between 0 and 1 (alpha can "
                     "be larger than 1).\n";
        exit(-1);
      }
...
void writeFile(const Image &image, std::string &filename, int u, int v) {
  ...
  for (unsigned int i = 0; i < pixels.size(); i++) {
    for (unsigned int j = 0; j < pixels[0].size(); j++) {
      file << pixels[i][j][0] << " " << pixels[i][j][1] << " "
           << pixels[i][j][2] << '\n';
    }
  }
...
void printUsage() {
  std::cout
      << "	Usage: ./raytracer <camera> <properties> <ply files...> "
         "<image>\n";
  std::cout << "	<camera> is a file containing the camera model\n";
  std::cout
      << "	<properties> is file containing the material properties of the "
         "model\n";
