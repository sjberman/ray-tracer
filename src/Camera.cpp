...
Camera::Camera(std::string &filename, std::vector<Model> &models,
  ...
  if (boundingBox[0] > boundingBox[2] || boundingBox[1] > boundingBox[3]) {
    std::cerr << "Minimum u and v coordinates must not be larger than maximum "
                 "u and v coordinates.\n";
    exit(-1);
  }
...
std::vector<float> Camera::solveLinearSystem(int j, std::vector<float> &L,
                                             std::vector<float> &U) {
...
std::vector<float> Camera::throwRay(std::vector<float> &L,
                                    std::vector<float> U) {
  ...
  if (found) {
    color = lightIntersect(addVector(L, scalarVectorMultiply(t, U)), faceIndex,
                           modelIndex, scalarVectorMultiply(-1, U));
  }
  return color;
}

// see if ray to polygon bounces and hits a light; if so, return color
std::vector<float> Camera::lightIntersect(std::vector<float> S, int faceIndex,
                                          int modelIndex,
                                          std::vector<float> V) {
  ...
  for (unsigned int i = 0; i < materials.size(); i++) {
    if (materials[i].objNum == modelIndex &&
        faceIndex >= materials[i].firstFace &&
        faceIndex <= materials[i].lastFace) {
  ...
  for (unsigned int i = 0; i < lights.size(); i++) {
    ...
    B[2] = lights[i].blue;
    tmp = addVector(
        scalarVectorMultiply(LN, vectorMatrixMultiply(K, B)),
        scalarVectorMultiply(pow(VR, alpha), scalarVectorMultiply(k, B)));
    I = addVector(tmp, I);
  }
  // recursive specular reflection
  /*std::vector<float> U = subtractVector(scalarVectorMultiply(2 * dotProduct(V,
  N), N), V);
  reflectCount++;
