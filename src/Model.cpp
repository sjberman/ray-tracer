...
Model::Model(std::vector<std::vector<float>> &vertices,
  ...
  for (unsigned int i = 0; i < faces.size(); i++) {
    centroid[0] = (vertices[faces[i][0]][0] + vertices[faces[i][1]][0] +
                   vertices[faces[i][2]][0]) /
                  3;
    centroid[1] = (vertices[faces[i][0]][1] + vertices[faces[i][1]][1] +
                   vertices[faces[i][2]][1]) /
                  3;
    centroid[2] = (vertices[faces[i][0]][2] + vertices[faces[i][1]][2] +
                   vertices[faces[i][2]][2]) /
                  3;
    float tmp = std::max(::distance(vertices[faces[i][0]], centroid),
                         ::distance(vertices[faces[i][1]], centroid));
  ...
  for (unsigned int i = 0; i < faces.size(); i++) {
    for (int j = 0; j < f2count - 2; j++) {
      faceNormal[i] =
          surfaceNormal(vertices[faces[i][j]], vertices[faces[i][j + 1]],
                        vertices[faces[i][j + 2]]);
      if (faceNormal[i][0] == 0 && faceNormal[i][1] == 0 &&
          faceNormal[i][2] == 0) {
        continue;
      } else
        break;
...
<< centroid[2] << " Radius: " << radius << std::endl;
        if(vertices.size() <= 3){
                return sphere;
        }
        std::vector<std::vector<float> > v1(vertices.begin(), vertices.begin() +
vertices.size()/2);
        std::vector<std::vector<float> > v2(vertices.begin() +
vertices.size()/2, vertices.end());
        sphere->parent = 1;
...
std::vector<std::vector<float>> Model::getVertices() { return vertices; }

std::vector<std::vector<float>> Model::getFaces() { return faces; }

std::vector<Sphere> Model::getSpheres() { return spheres; }
...
void Model::rotate(float x, float y, float z, float theta) {
  ...
  rotationMatrix[0][1] =
      (x * y * (1 - cos(angle)) - z * sqrt(L) * sin(angle)) / L;
  rotationMatrix[0][2] =
      (x * z * (1 - cos(angle)) + y * sqrt(L) * sin(angle)) / L;

  rotationMatrix[1][0] =
      (x * y * (1 - cos(angle)) + z * sqrt(L) * sin(angle)) / L;
  rotationMatrix[1][1] = (v + (u + w) * cos(angle)) / L;
  rotationMatrix[1][2] =
      (y * z * (1 - cos(angle)) - x * sqrt(L) * sin(angle)) / L;
  ...
  setTmpVertices(true);
  tmpVertices =
      transpose(matrixMultiply(rotationMatrix, transpose(tmpVertices)));
  setTmpVertices(false);
