...
class Camera {
public:
  Camera(std::string &filename, std::vector<Model> &models,
         std::vector<Light> &lights, std::vector<Material> &materials);
  virtual ~Camera();
  std::vector<float> solveLinearSystem(int j, std::vector<float> &L,
                                       std::vector<float> &U);
  std::vector<float> throwRay(std::vector<float> &L, std::vector<float> U);
  std::vector<float> lightIntersect(std::vector<float> S, int faceIndex,
                                    int modelIndex, std::vector<float> V);
