...
void Image::generate(){
	...
	for(unsigned int i = 0; i < pixels.size(); i++){
		for(unsigned int j = 0; j < pixels[0].size(); j++){
			...
			//calculate pixel point L, throw 5 rays through each pixel for anti-aliasing
			float random = 0;
			for(int k = 0; k < 5; k++){
				random = ((float)rand())/RAND_MAX - 0.5;
				float a = (float)(x + random);
				float b = (float)(y + random);
				L = addVector(addVector(addVector(camera.focalPoint, scalarVectorMultiply(camera.focalLength, camera.n)), scalarVectorMultiply(a, camera.u)), scalarVectorMultiply(b, camera.v));
				camera.reflectCount = 0;
				camera.refractCount = 0;
				#pragma omp ordered
				tmp = addVector(camera.throwRay(L, normalize(subtractVector(L, camera.focalPoint))), tmp);
			}
			tmp[0] = tmp[0]/5; tmp[1] = tmp[1]/5; tmp[2] = tmp[2]/5;
			color = tmp;

			/*L = addVector(addVector(addVector(camera.focalPoint, scalarVectorMultiply(camera.focalLength, camera.n)), scalarVectorMultiply(x, camera.u)), scalarVectorMultiply(y, camera.v));
			camera.reflectCount = 0;
			camera.refractCount = 0;
			#pragma omp ordered
			color = camera.throwRay(L, normalize(subtractVector(L, camera.focalPoint)));*/
